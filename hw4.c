...
static ssize_t homework3_read(struct file *file, char __user *buf, size_t len,
                              loff_t *offset) {
  int ret;
...
static ssize_t homework3_write(struct file *file, const char __user *buf,
  ...
  *offset = 0;
  ...
  writel(user_input, intel->hw_addr + LED_CTRL);
  led_value = readl(intel->hw_addr + LED_CTRL);
...
static int dev_probe(struct pci_dev *pdev, const struct pci_device_id *ent) {
  ...
  led_conf = readl(intel->hw_addr + 0x0);
  dev_info(&pdev->dev, "led_conf = 0x%08x\n", led_conf);

  led_conf = readl(intel->hw_addr + LED_CTRL);
  dev_info(&pdev->dev, "led_conf = 0x%08x\n", led_conf);
